...
,
  {
   ...
   answer: '最常见思路不是追求“绝对同时成功”，而是保证最终一致。\n\n常见方案：\n1. 本地消息表\n- 业务数据和消息记录放在同一个本地事务里提交\n- 后台任务扫描未发送消息并重试投递\n- 成功后把消息状态改为已发送\n\n2. 事务消息\n- 比如 RocketMQ 事务消息\n- 先发半消息，再执行本地事务，再根据结果提交或回滚消息\n\n答题重点：\n- 先把业务和“待发送消息记录”绑定在一个本地事务里\n- 再通过重试、回查、补偿保证消息最终送达\n- 真正线上最怕的是“数据库成功了，消息没出去”。',
   analogy: '这像先在账本上登记“这件快递必须寄出”，即使快递员一时没来，后面也会有人根据账本继续补寄。',
   importance: 'high'
  },
  {
   id: 'mq-11',
   question: 'MQ 消费方法上直接加 @Transactional 要注意什么？',
   answer: '很多人以为加了 \`@Transactional\` 就万事大吉，但这题真正考的是边界。\n\n注意点：\n1. 本地事务只管数据库，不等于 MQ ack 也自动受控\n2. 如果业务提交成功但 ack 失败，消息可能重复投递\n3. 如果先 ack 再执行业务，一旦业务失败就会丢消息\n4. 所以通常要把“业务成功后再确认消费”这个顺序讲清楚\n\n工程实践里常见做法：\n- 手动 ack\n- 业务逻辑做幂等\n- 失败走重试或死信队列\n- 不把 \`@Transactional\` 当成消息一致性的全部答案',
   analogy: '数据库事务像把屋里账记对了，但 MQ ack 像门口签收单，屋里账记对不代表门口那张签收单一定同步处理好了。',
   importance: 'high'
  }
];
